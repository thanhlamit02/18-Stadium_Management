...
namespace BusinessLayer.Repository
{
    ...
    {
        public IEnumerable<RentOrderFormatVM> GetList(int Bill_Id, string StadiumName)
        {
            ...
            {
                yield return new RentOrderFormatVM
                {
                    Id = s.Id,
                    Bill_Id = s.Bill_Id,
                    Bill_Code = s.Bill.BillCode,
                    Stadium_Id = s.Stadium_Id,
                    Stadium_Name = s.Stadium.Name,
                    Stadium_Price = s.Stadium.Price.ToString("N0"),
                    StartRentDate = s.StartRentDate,
                    EndRentDate = s.EndRentDate,
